...
@section('content')
    ...
    <div class="row" style="height: 35rem;">
        <div class="d-flex justify-content-center align-items-center">
            @if(empty($applyAll))
                <h3 class="">Are you sure you want to unsubscribe player "{{ $player->first_name . ' '. $player->last_name }}" from team "{{ $team->name }}"?</h3>
            @else
        ...
        <div class="d-flex justify-content-center gap-5">
                ...
                <form action="/team/unsubscribe-all" method="post">
                    @csrf
                    <button type="submit" name="team_id" value="{{ $team->id }}" class="btn btn-lg btn-success">Yes</button>
                @endif
            </form>
            <div>
